...
# py_trees behaviour tree for: arm_handover
import py_trees
...
def create_tree(runtime) -> py_trees.behaviour.Behaviour:
    """Build the 'arm_handover' tree bound to ``runtime`` and return its root."""
    return _Guarded('guarded',
        py_trees.composites.Sequence(
            name='sequence', memory=True,
                children=[
                    _FSMEvent('right_arm.PICK', runtime, 'right_arm', 'PICK', 'PICK_DONE', 'event'),
                    _FSMEvent('right_arm.HANDOVER', runtime, 'right_arm', 'HANDOVER', 'HANDOVER_DONE', 'event'),
                    _FSMEvent('left_arm.TAKE_POSE', runtime, 'left_arm', 'TAKE_POSE', 'TAKE_POSE_DONE', 'event'),
                    _FSMEvent('left_arm.PLACE', runtime, 'left_arm', 'PLACE', 'PLACE_DONE', 'event'),
                ],),
        pre=[],
        post=[(py_trees.common.Status.FAILURE, lambda: _bb_set('handover_failed', True)), (py_trees.common.Status.SUCCESS, lambda: _bb_set('handover_complete', True))])
