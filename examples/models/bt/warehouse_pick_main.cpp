...
class WarehousePickRuntime final : public warehouse_pick::WarehousePickRuntime {
 public:
  ...
  BT::NodeStatus on_detect_object(BT::TreeNode& node) override {
    ...
    return node.setOutput("pose", pick_pose_) ? BT::NodeStatus::SUCCESS : BT::NodeStatus::FAILURE;
  }
  ...
  BT::NodeStatus on_close_gripper(BT::TreeNode& node) override {
    double width;
    if (!read(node, "width", width) || width <= 0.0 || target_.empty()) return BT::NodeStatus::FAILURE;
    gripper_open_ = false;
  ...
  BT::NodeStatus on_holding(BT::TreeNode& node) override {
    std::string object;
    return read(node, "object", object) && object == held_object_
               ? BT::NodeStatus::SUCCESS
               : BT::NodeStatus::FAILURE;
  }
 ...
 private:
  template <typename T>
  static bool read(BT::TreeNode& node, const char* port, T& value) {
...
int main(int argc, char* argv[]) {
  ...
  return tree.tickWhileRunning() == BT::NodeStatus::SUCCESS ? 0 : 1;
}
