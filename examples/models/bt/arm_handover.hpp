...
#ifndef ARM_HANDOVER_BT_HPP
#define ARM_HANDOVER_BT_HPP
...
{
  public:
    FSMEventNode(const std::string &name, const BT::NodeConfig &config, ArmHandoverRuntime *runtime)
  ...
  private:
    ...
    ArmHandoverRuntime *runtime_;
    std::string         instance_;
    int                 command_ = -1, await_index_ = -1, fail_index_ = -1;
    bool                await_is_state_ = false, fail_is_state_ = false;
};

inline void register_nodes(BT::BehaviorTreeFactory &factory, ArmHandoverRuntime &runtime)
{
