...
#ifndef ASYNC_PICK_BT_HPP
#define ASYNC_PICK_BT_HPP
...
class AsyncPickRuntime
{
  public:
    AsyncPickRuntime()
    {
        ...
        fsm_gripper_   = gripper::create_fsm();
    }
    virtual ~AsyncPickRuntime()
    {
        right_arm::destroy_fsm(fsm_right_arm_);
        gripper::destroy_fsm(fsm_gripper_);
    }
    ...
    // Advance the gripper controller one tick; produce completion events
    // (e.g. *_DONE) onto fsm->eventData once its current sub-behaviour finishes.
    virtual void step_gripper(struct fsm_nbx *fsm) = 0;

    struct fsm_nbx *fsm_of(const std::string &instance)
    {
        if (instance == "right_arm") return fsm_right_arm_;
        if (instance == "gripper") return fsm_gripper_;
        return nullptr;
    }
    ...
    int event_index(const std::string &instance, const std::string &event)
    {
        ...
        if (instance == "gripper") {
            if (event == "GRASP") return gripper::GRASP;
            if (event == "GRASPED_OK") return gripper::GRASPED_OK;
            if (event == "GRASP_FAULT") return gripper::GRASP_FAULT;
        }
        return -1;
    }
    ...
    int state_index(const std::string &instance, const std::string &state)
    {
        ...
        if (instance == "gripper") {
            if (state == "IDLE") return gripper::IDLE;
            if (state == "GRASPING") return gripper::GRASPING;
            if (state == "GRASPED") return gripper::GRASPED;
            if (state == "FAULT") return gripper::FAULT;
        }
        return -1;
    }
  ...
  private:
    ...
    struct fsm_nbx *fsm_gripper_   = nullptr;
};
...
{
  public:
    FSMEventNode(const std::string &name, const BT::NodeConfig &config, AsyncPickRuntime *runtime)
  ...
  private:
    ...
    AsyncPickRuntime *runtime_;
    std::string       instance_;
    int               command_ = -1, await_index_ = -1, fail_index_ = -1;
    bool              await_is_state_ = false, fail_is_state_ = false;
};

inline void register_nodes(BT::BehaviorTreeFactory &factory, AsyncPickRuntime &runtime)
{
