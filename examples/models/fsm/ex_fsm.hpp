...
static constexpr const char *STATE_URIS[NUM_STATES] = {
    "http://example.org/S_START",   "http://example.org/S-CONFIGURE", "http://example.org/S_IDLE",
    "http://example.org/S-COMPILE", "http://example.org/S_EXECUTE",   "http://example.org/S_EXIT",
};
...
static constexpr const char *EVENT_URIS[NUM_EVENTS] = {
    "http://example.org/E-CONFIGURE-ENTERED", "http://example.org/E-CONFIGURE-EXIT",
    "http://example.org/E_IDLE_ENTERED",      "http://example.org/E_IDLE_EXIT_EXECUTE",
    "http://example.org/E_IDLE_EXIT_COMPILE", "http://example.org/E-COMPILE-ENTERED",
    "http://example.org/E-COMPILE-EXIT",      "http://example.org/E_EXECUTE_ENTERED",
    "http://example.org/E_EXECUTE_EXIT",      "http://example.org/e-step",
};
...
static constexpr const char *TRANSITION_URIS[NUM_TRANSITIONS] = {
    "http://example.org/T_START_CONFIGURE", "http://example.org/T_CONFIGURE_IDLE",
    "http://example.org/T_IDLE_IDLE",       "http://example.org/T_IDLE_EXECUTE",
    "http://example.org/T_IDLE_COMPILE",    "http://example.org/T_COMPILE_EXECUTE",
    "http://example.org/T_EXECUTE_EXECUTE", "http://example.org/T_EXECUTE_IDLE",
};
...
inline struct fsm_nbx *create_fsm()
{
    ...
    // sm states
    struct state *states = new (std::nothrow)
      state[NUM_STATES]{ { .name = "S_start" },   { .name = "S_configure" }, { .name = "S_idle" },
                         { .name = "S_compile" }, { .name = "S_execute" },   { .name = "S_exit" } };
    ...
    struct event_reaction *reactions = new (std::nothrow) event_reaction[NUM_REACTIONS]{
        ...
        },
        {
          ...
          .numFiredEvents      = 2,
          .firedEventIndices   = new unsigned int[2]{ E_CONFIGURE_ENTERED, E_STEP },
        },
        {
          .conditionEventIndex = E_STEP,
