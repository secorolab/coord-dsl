...
Scope: composites, the cleanly-mapping decorators, action/condition leaves and
FSM-coordination (``send/await``) nodes. Guards, scripted builtins and the
composites without a py_trees analogue (``if-then-else``, ``while-do-else``,
``switch``) raise ``NotImplementedError`` -- the C++/XML backend covers those.
"""
...
def _has_guards(g, node):
    return next(g.objects(node, NS_BT.guard), None) is not None
...
def _render(g, node):
    if _has_guards(g, node):
        raise NotImplementedError(
            "node guards ([skip-if], [failure-if], [on-success], ...) are not supported "
            "by the py_trees target; use the C++/XML backend for scripted guards"
        )
